...
class MessageHandler(AES, RSA):
    ...
    def send_messages(self, messages:list[Message], sender_signature:Signature, reciever_public_key_pem:bytes, users_count:int):
        ...
        for message in real_and_fake_messages:
            cipher_text = self.encrypt(message['message_text'], message['key'], message['iv'])
            file_name = self.encrypt_with_public_key(message['reciever_public_key_pem'], message['key'] + message['iv'])
            path = Path('files/messages') / f"{file_name}.txt"
            with open(path) as f:
